...
/* Tear down the resources main() owns directly. The guest_t, vCPU, and
 * GDB stub are owned by elfuse_launch and freed inside its return path;
 * this function only walks back over things main() allocated before the
 * launch call (heap argv copy, sysroot path, the --create-sysroot
 * sparsebundle mount) and restores the host cwd elfuse may have left
 * mutated.
 */
...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < guest_argc; i++) {
        ...
        if (!guest_argv[i]) {
            log_error("out of memory");
            cleanup_main_resources(&sysroot_mount,
    ...
    /* Proctitle rewriting clobbers the original argv block. It runs here,
     * in main(), because elfuse_launch only has the heap-copied guest_argv
     * (the strdup'd shadow) -- the postgres/nginx argv-clobber trick needs
     * the live argv pointer the kernel handed in.
     */
    runtime_set_process_title(argc, argv, elf_path);

    launch_args_t launch = {
        .elf_path = elf_path,
        .sysroot = have_sysroot ? sysroot : NULL,
    ...
    int exit_code = elfuse_launch(&launch);
