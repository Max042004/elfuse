 ...
 * positional-ELF CLI) and the Phase 3 oci run orchestrator. The function
 ...
 *     via proc_set_ids.
 *   - cwd_guest reserves a slot for Phase 3 commit 5 (oci run) to set
...
typedef struct {
    ...
    const char **envp;
    /* Override host uid/gid when true. Phase 3 commit 5 sets this from
     * the image User field; main()'s legacy path leaves it false.
     */
    bool has_creds;
    uint32_t uid;
    uint32_t gid;
    /* Guest-absolute initial working directory. NULL inherits the host
     * cwd. Wired up by Phase 3 commit 5.
     */
