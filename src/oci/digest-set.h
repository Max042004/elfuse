 ...
 * Plan 1 garbage collector to accumulate the reachable blob set across
 * pin walks, image-config blobs, and unpacked-sysroot origin sidecars.
 *
 ...
 * cheap. The C1.3 sweep walks blobs/<algo>/<hex> and calls contains()
 * once per blob, which bsearch makes O(log n). If profiling later
