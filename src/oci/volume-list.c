 ...
 * (used by Plan 1 garbage collection and Plan 4 status reporting) free
 * of the sparsebundle / case-sensitivity provisioning chain pulled in
...
static int append_path(oci_volume_list_t *list, size_t *cap, char *path,
                       const char **err)
{
    if (list->count == *cap) {
