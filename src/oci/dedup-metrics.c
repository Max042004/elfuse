 ...
 * digest. Walks one level of image-index indirection (linux/arm64 pick).
 * Returns a heap-allocated config digest string on success (caller frees),
 * or NULL with errno set when the manifest is missing, malformed, or the
 * index has no linux/arm64 entry whose sub-manifest blob is on disk.
 */
                                   ...
                                   const char *manifest_digest)
{
     ...
     * but resolve_config_digest handles arbitrary nesting safely because
     * the recursion terminates when the body parses as an image-manifest.
     */
    char *cfg = resolve_config_digest(store, sub_digest);
    free(sub_digest);
    return cfg;
}
                     ...
                     size_t *compared_images)
{
     ...
     */
    char *config_digest = resolve_config_digest(store, pin_manifest_digest);
    if (!config_digest)
        return;
                              ...
                              const char **err)
{
    ...
    *err = NULL;
     ...
     */
    char *target_config = resolve_config_digest(store, target_manifest_digest);
    if (!target_config) {
        int saved = errno;
