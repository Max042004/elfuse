...
static _Thread_local char rf_err_buf[512];
...
static void set_err_fmt(const char **err, const char *fmt, ...)
{
    va_list ap;
    va_start(ap, fmt);
    vsnprintf(rf_err_buf, sizeof(rf_err_buf), fmt, ap);
    va_end(ap);
    if (err)
        *err = rf_err_buf;
}
...
static int ensure_etc_dir(const char *run_dir, const char **err)
{
    ...
    if (n < 0 || (size_t) n >= sizeof(etc_path)) {
        set_err_static(err, "runtime-files: <run_dir>/etc path overflow");
        errno = ENAMETOOLONG;
        return -1;
    }
    ...
    if (errno != EEXIST) {
        int saved = errno;
        set_err_fmt(err, "runtime-files: mkdir %s failed: %s", etc_path,
                    strerror(saved));
        errno = saved;
        return -1;
    }
    ...
    if (lstat(etc_path, &st) < 0) {
        int saved = errno;
        set_err_fmt(err, "runtime-files: lstat %s failed: %s", etc_path,
                    strerror(saved));
        errno = saved;
        return -1;
    }
    if (!S_ISDIR(st.st_mode)) {
        set_err_fmt(err, "runtime-files: %s exists but is not a directory",
                    etc_path);
        errno = ENOTDIR;
        return -1;
    }
                              ...
                              const char **err)
{
    ...
    if (n < 0 || (size_t) n >= sizeof(path)) {
        set_err_fmt(err, "runtime-files: path for /etc/%s too long", leaf);
        errno = ENAMETOOLONG;
        return -1;
    }
    if (unlink(path) < 0 && errno != ENOENT) {
        int saved = errno;
        set_err_fmt(err, "runtime-files: unlink %s failed: %s", path,
                    strerror(saved));
        errno = saved;
        return -1;
    }
    ...
    if (fd < 0) {
        int saved = errno;
        set_err_fmt(err, "runtime-files: open %s failed: %s", path,
                    strerror(saved));
        errno = saved;
        return -1;
    }
...
static int write_resolv_conf(const char *run_dir, const char **err)
{
    ...
    if (rc < 0) {
        set_err_fmt(err, "runtime-files: write resolv.conf failed: %s",
                    strerror(saved));
        close(fd);
        errno = saved;
        return -1;
    ...
    if (close(fd) < 0) {
        int e = errno;
        set_err_fmt(err, "runtime-files: close resolv.conf failed: %s",
                    strerror(e));
        errno = e;
        return -1;
    }
...
static int write_hosts(const char *run_dir, const char **err)
{
    ...
    if (write_full(fd, body, sizeof(body) - 1) < 0) {
        int saved = errno;
        set_err_fmt(err, "runtime-files: write hosts failed: %s",
                    strerror(saved));
        close(fd);
        errno = saved;
        return -1;
    ...
    if (close(fd) < 0) {
        int e = errno;
        set_err_fmt(err, "runtime-files: close hosts failed: %s", strerror(e));
        errno = e;
        return -1;
    }
    return 0;
...
static int write_hostname(const char *run_dir, const char **err)
{
    ...
    if (write_full(fd, body, sizeof(body) - 1) < 0) {
        int saved = errno;
        set_err_fmt(err, "runtime-files: write hostname failed: %s",
                    strerror(saved));
        close(fd);
        errno = saved;
        return -1;
    ...
    if (close(fd) < 0) {
        int e = errno;
        set_err_fmt(err, "runtime-files: close hostname failed: %s",
                    strerror(e));
        errno = e;
        return -1;
    }
...
int oci_runtime_files_inject(const char *run_dir, const char **err)
{
    if (err)
        *err = NULL;
    if (!run_dir || !*run_dir) {
        set_err_static(err, "runtime-files: NULL/empty run_dir");
        errno = EINVAL;
        return -1;
    }
