 ...
 *
 * Phase 2 commits Q2 of oci-roadmap.md to APFS clonefile-based
 * copy-up: each `elfuse oci clone` invocation gets a fresh directory
 * tree cloned from the immutable image sysroot. APFS file-level CoW
 * makes the clone nearly O(1) at start, and only modified files
 * allocate new blocks.
 *
 ...
 * created, so the layer applier in commit 5 must materialize intra-
 * image hardlinks via link(2) rather than relying on the clone.
 *
 ...
 * (epoch seconds). Phase 2 ships this as a stub returning 0 because
 * `elfuse oci prune` is not extended in this PR; Phase 3 will
 * actually walk the directory.
 */
