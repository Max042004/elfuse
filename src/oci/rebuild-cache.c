...
/* Recursive rmdir / unlink for clean-up after a failed snapshot stage.
 * Duplicates the rm_recursive in src/oci/unpack.c because pulling unpack.c
 * into a small back-fill module would drag the full layer-apply / decompress
 * graph along for one helper. Lift to a shared util if a third copy
 * appears.
 */
 ...
 */
static int compute_terminal_chain(char *const *diff_ids, size_t n,
                                  char *out, size_t cap)
{
 ...
 */
static int snapshot_tree_to_stack(oci_store_t *store, const char *tree_path,
                                  const char *chain_id, const char **err)
{
    ...
    int n = snprintf(origin_path, sizeof(origin_path),
                     "%s/.elfuse-origin.json", stage_path);
                      ...
                      const char **err)
{
    ...
    *err = NULL;
    ...
    if (oci_volume_list_unpacked(volume_root, &trees, &list_err) < 0) {
        ...
        set_err(err,
                list_err ? list_err
                         : "rebuild-cache: list unpacked failed",
                saved);
        return -1;
    }
    ...
    for (size_t i = 0; i < trees.count; i++) {
        ...
        if (compute_terminal_chain(origin.layer_diffids, n_diffs, chain,
                                   sizeof(chain)) < 0) {
            fprintf(stderr,
                    "rebuild-cache: %s: chainid compute failed: %s\n",
                    tree, strerror(errno));
            opts->trees_failed++;
            oci_origin_free(&origin);
            continue;
        ...
        if (has < 0) {
            fprintf(stderr,
                    "rebuild-cache: %s: stack_has probe failed: %s\n",
                    tree, strerror(errno));
            opts->trees_failed++;
            continue;
        }
        ...
        if (snapshot_tree_to_stack(store, tree, chain, &snap_err) < 0) {
            fprintf(stderr, "rebuild-cache: %s: %s: %s\n", tree,
                    snap_err ? snap_err : "snapshot failed",
                    strerror(errno));
            opts->trees_failed++;
            continue;
        }
