 ...
 *
 * Plan 3 C3.3c introduced the ChainID-keyed stack cache at
 ...
 * unpacked before C3.3c landed (or that was unpacked into a store whose
 * C3.3b schema marker had just wiped v1 entries) leaves no stack snapshot
 * on disk: the next unpack still pays the full extract cost even though a
 * usable assembled stage_dir already sits under <volume>/images/sha256-<hex>/.
 *
...
typedef struct {
    ...
    size_t trees_skipped_bad_origin;    /* origin sidecar parse or schema failure */
