...
static bool valid_path_component(const char *s, size_t len)
{
    ...
    while (i < len) {
        ...
        /* Separator run: a single '.', '-', '_', or exactly "__". Anything
         * else is rejected so paths like "a..b" or "a___b" do not slip
         * through.
         */
...
static bool valid_digest(const char *s, size_t len, const char **err_msg)
{
    ...
    if (algo_len == 6 && memcmp(s, "sha256", 6) == 0) {
        ...
        set_err(err_msg,
                "digest algorithm must be sha256 or sha512");
        return false;
    }
...
int oci_ref_parse(const char *input, oci_ref_t *out, const char **err_msg)
{
    ...
    if (at) {
        ...
        const char *second = memchr(at + 1, '@', total - (size_t) (at + 1 - input));
        if (second) {
    ...
    const char *tag_colon = memchr(scan_from, ':',
                                   (size_t) (scan_end - scan_from));
    if (tag_colon) {
