 ...
 * src/oci/dedup-metrics.c and src/oci/store.c. The Plan 3 memory documents
 * the lift threshold (deferred until a fourth caller of the diff_id walker
 * pattern appears); status.c is the third caller, still below the cutoff.
 */
...
/* Resolve a manifest blob to its image-config digest. Walks one level of
 * image-index indirection (linux/arm64 pick). Returns a heap-allocated
 * "<algo>:<hex>" config digest on success (caller frees) or NULL with errno
 * set on failure:
                                   ...
                                   oci_status_pin_code_t *out_status_code)
{
    ...
    char *cfg = resolve_config_digest(store, sub_digest, &sub_code);
    free(sub_digest);
                              ...
                              const char *digest_str)
{
    ...
    if (!algo_name)
        return false;
    char path[STATUS_PATH_MAX];
    int n = snprintf(path, sizeof(path), "%s/%s/%s/%s", root, base, algo_name,
                     hex);
    if (n < 0 || (size_t) n >= sizeof(path))
        return false;
    struct stat st;
                           ...
                           oci_digest_set_t *chain_acc)
{
     ...
     */
    if (stat_blob(blobs, out->digest, &out->manifest_size, &out->last_seen_mtime) <
        0) {
        out->manifest_size = 0;
        out->last_seen_mtime = -1;
        out->status = OCI_STATUS_PIN_MISSING_MANIFEST;
    ...
    char *config_digest = resolve_config_digest(store, out->digest,
                                                &resolve_code);
    if (!config_digest) {
                                ...
                                oci_digest_set_t *chain_acc)
{
    ...
    if (oci_origin_read(tree_path, &origin, NULL) < 0) {
        out->status = (errno == ENOENT)
                          ? OCI_STATUS_UNPACKED_MISSING_ORIGIN
    ...
    if (volume_root) {
        ...
        if (oci_volume_list_unpacked(volume_root, &trees, NULL) == 0 &&
            ...
            for (size_t i = 0; i < trees.count; i++) {
                walk_unpacked_entry(trees.items[i], skip_du,
                                    &out->unpacked[i], &diff_acc, &chain_acc);
            }
        }
        oci_volume_list_free(&trees);
    }
