 ...
 * LDAP) are intentionally out of scope: the roadmap (oci-roadmap.md Q4)
 * caps Phase 4 at static-passwd resolution.
 *
...
/* Diagnostic scratch. Thread-local so concurrent oci_user_lookup callers
 * (one per --keep run dir on a future multiplexed oci run) do not clobber
 * each other's err pointer. 512 B fits the longest dynamic message: the
 * rejected token plus a path plus errno text.
 */
static _Thread_local char ul_err_buf[512];
...
__attribute__((format(printf, 2, 3))) static void set_err_fmt(const char **err,
                                                              const char *fmt,
                                                              ...)
{
    va_list ap;
    va_start(ap, fmt);
    vsnprintf(ul_err_buf, sizeof(ul_err_buf), fmt, ap);
    va_end(ap);
    if (err)
        *err = ul_err_buf;
}
...
{
    if (n == 0) {
        set_err_static(err, "user-lookup: empty name token");
        return -1;
    }
    if (n > USER_NAME_MAX) {
        set_err_fmt(err, "user-lookup: name token exceeds %d bytes",
                    USER_NAME_MAX);
        return -1;
    }
    for (size_t i = 0; i < n; i++) {
                             ...
                             const char **err)
{
    ...
    if (fd < 0) {
        int saved = errno;
        set_err_fmt(err, "user-lookup: open %s: %s", path, strerror(saved));
        errno = saved;
        return -1;
    }
    struct stat st;
    if (fstat(fd, &st) < 0) {
        int saved = errno;
        set_err_fmt(err, "user-lookup: fstat %s: %s", path, strerror(saved));
        close(fd);
        errno = saved;
        return -1;
    }
    if (!S_ISREG(st.st_mode)) {
        set_err_fmt(err, "user-lookup: %s is not a regular file", path);
        close(fd);
        errno = EINVAL;
        return -1;
    }
    if ((uint64_t) st.st_size > USER_DB_MAX_BYTES) {
        set_err_fmt(err, "user-lookup: %s exceeds %u-byte cap", path,
                    USER_DB_MAX_BYTES);
        close(fd);
        errno = EFBIG;
        return -1;
    ...
    if (!buf) {
        set_err_static(err, "user-lookup: out of memory reading database");
        close(fd);
        errno = ENOMEM;
        return -1;
    }
    ...
    while (off < len) {
        ...
        if (n < 0) {
            ...
            int saved = errno;
            free(buf);
            set_err_fmt(err, "user-lookup: read %s: %s", path, strerror(saved));
            close(fd);
            errno = saved;
            return -1;
        }
                                 ...
                                 const char **err)
{
    ...
    if (!path) {
        set_err_static(err, "user-lookup: out of memory");
        errno = ENOMEM;
        return -1;
    }
    char *buf = NULL;
    ...
    free(buf);
    set_err_fmt(err, "user-lookup: user '%.*s' not found in /etc/passwd",
                (int) name_len, name);
    errno = ENOENT;
    return -1;
}
                                ...
                                const char **err)
{
    ...
    free(buf);
    set_err_fmt(err, "user-lookup: group '%.*s' not found in /etc/group",
                (int) name_len, name);
    errno = ENOENT;
    return -1;
}
    ...
    *primary_gid_set = false;
    if (is_all_digits(s, n)) {
        if (parse_u32(s, n, uid_out) < 0) {
            set_err_fmt(err, "user-lookup: uid '%.*s' overflows uint32",
                        (int) n, s);
            errno = EINVAL;
            return -1;
        }
                               ...
                               const char **err)
{
    if (is_all_digits(s, n)) {
        if (parse_u32(s, n, gid_out) < 0) {
            set_err_fmt(err, "user-lookup: gid '%.*s' overflows uint32",
                        (int) n, s);
            errno = EINVAL;
            return -1;
        }
                    ...
                    const char **err)
{
    ...
    if (!user_str || !*user_str) {
        set_err_static(err, "user-lookup: empty User string");
        errno = EINVAL;
        return -1;
    }
    if (!uid_out || !gid_out) {
        set_err_static(err, "user-lookup: NULL uid_out / gid_out");
        errno = EINVAL;
        return -1;
    }
    ...
    if (uid_len == 0) {
        set_err_fmt(err, "user-lookup: empty uid token in '%s'", user_str);
        errno = EINVAL;
        return -1;
    }
    if (colon && gid_len == 0) {
        set_err_fmt(err, "user-lookup: empty gid token in '%s'", user_str);
        errno = EINVAL;
        return -1;
    }
