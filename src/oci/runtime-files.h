 ...
 * ask the runtime to overlay three files on the per-run clone-rootfs
 * so guest libc lookups (getaddrinfo, gethostname, /etc/hosts walks)
 * see values matching the macOS host rather than the image's
 * containerd defaults. The entry point is invoked from src/oci/run.c
