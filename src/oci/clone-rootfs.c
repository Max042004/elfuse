                        ...
                        const char **err)
{
    ...
    if (err)
        *err = NULL;
    /* Phase 2 stub: `elfuse oci prune` does not yet sweep stale runs.
     * Phase 3 will walk volume_root/runs/ and unlink entries older
     * than older_than. The stub returns 0 so the CLI can call it
