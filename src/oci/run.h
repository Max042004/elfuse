 ...
 *
 * Closes the Phase 3 loop: unpack + clone-rootfs + image-config parse +
 ...
 *
 * Dependencies (all already landed in earlier slices):
 *
 *   - oci_unpack (Phase 2)            -- layers -> image sysroot under
 *                                        the APFS sysroot volume
 *   - oci_clone_rootfs (Phase 2)      -- clonefile-based per-run rootfs
 *   - oci_image_config_parse (Phase 1)
 *   - oci_runspec_build (Phase 3 C2)  -- folds image config + CLI flags
 *                                        into argv/envp/cwd/uid bundle
 *   - oci_path_resolve (Phase 3 C3)   -- argv0 + PATH -> host_path/
 *                                        guest_path with sysroot
 *                                        containment
 *   - elfuse_launch (Phase 3 C4)      -- VM bring-up shared with main()
 *
 ...
 * run-dir naming option that Phase 2's oci_clone_rootfs does not yet
 * support, so the field is currently ignored. spec carries every
