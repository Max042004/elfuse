 ...
 * separate metrics (raw vs stack) because the Plan 3 caches dedup along
 * different axes; collapsing them into one number would hide which family
 * needs operator attention.
 *
 * Plan 4 C4.1 first consumer. Reusable bits to revisit if Plan 4 C4.2 or any
...
typedef enum {
    ...
    OCI_STATUS_PIN_CORRUPT_MANIFEST = 2,    /* blob unparseable as manifest or index */
    OCI_STATUS_PIN_CORRUPT_CONFIG = 3,      /* image-config blob missing or unparseable */
    OCI_STATUS_PIN_INDEX_NO_ARM64 = 4,      /* image-index has no linux/arm64 entry */
...
typedef struct {
    ...
    int64_t last_seen_mtime;   /* st_mtime of manifest blob (epoch sec); -1 if missing */
