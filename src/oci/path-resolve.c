...
/* Diagnostic scratch shared across the module. Thread-local so a future
 * multiplexed oci run that probes several launches in parallel does not
 * trample the err pointer between calls.
 */
static _Thread_local char path_err_buf[1024];
...
static void set_err_fmt(const char **err, const char *fmt, ...)
{
    va_list ap;
    va_start(ap, fmt);
    vsnprintf(path_err_buf, sizeof(path_err_buf), fmt, ap);
    va_end(ap);
    if (err)
        *err = path_err_buf;
}
                          ...
                          const char **err)
{
    ...
    if (!guest_path) {
        set_err_static(err, "out of memory building guest path");
        errno = ENOMEM;
        return -1;
    }
    ...
    if (!host_path) {
        free(guest_path);
        set_err_static(err, "out of memory building host path");
        errno = ENOMEM;
        return -1;
    }
    ...
    if (probe == PROBE_NOEXEC) {
        set_err_fmt(err, "'%s' is not executable inside sysroot", argv0);
        errno = EACCES;
    } else if (probe == PROBE_ESCAPE) {
        set_err_fmt(err, "'%s' resolves outside sysroot (refusing escape)",
                    argv0);
        errno = ENOENT;
    } else {
        set_err_fmt(err, "cannot find '%s' inside sysroot", argv0);
        errno = ENOENT;
    }
    free(host_path);
    free(guest_path);
                               ...
                               const char **err)
{
    ...
    while (*p) {
        ...
        if (!guest_dir) {
            set_err_static(err, "out of memory building PATH entry");
            errno = ENOMEM;
            goto cleanup;
        }
        ...
        if (!guest_path || !host_path) {
            ...
            free(host_path);
            free(guest_dir);
            set_err_static(err, "out of memory building host candidate");
            errno = ENOMEM;
            goto cleanup;
        }
    ...
    oom_in_loop:
        free(host_path);
        free(guest_dir);
        set_err_static(err, "out of memory recording PATH entry");
        errno = ENOMEM;
        goto cleanup;
    }
                     ...
                     const char **err)
{
    ...
    if (!sysroot_dir || !argv0 || !*argv0 || !out_host_path ||
        !out_guest_path) {
        set_err_static(err, "oci_path_resolve: NULL argument or empty argv0");
        errno = EINVAL;
        return -1;
    }
    ...
    if (!real_sysroot) {
        set_err_fmt(err, "sysroot not accessible: %s", sysroot_dir);
