 ...
 *
 * PATH search and rootfs materialization happen in later Phase 3 modules
 * (path-resolve, run). This builder treats argv[0] as opaque; the caller
 * downstream is responsible for resolving the host path the guest will
 * actually load.
 *
 * Override matrix (issue #31 acceptance 2) follows the table documented in
 * the Phase 3 plan: --entrypoint replaces the image Entrypoint and the
 * image Cmd is dropped whenever CLI positional arguments are provided or
 * --entrypoint is set. The "image has neither Entrypoint nor Cmd and the
 * CLI supplied no positional arguments" case is the only hard-fail in the
 * argv assembly path.
 *
