 ...
 * resolved target would escape the unpack root. The unpack-time check
 * uses oci_path_join_safe + oci_symlink_target_check so a malicious
 * tar that smuggles a relative `..` through the prefix slot cannot
 * touch host paths.
 *
 * Whiteouts:
 ...
 * socket entries are rejected with ENOTSUP, matching the asymmetric
 * subset in oci-roadmap.md Q3.
 *
