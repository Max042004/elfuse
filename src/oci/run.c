 ...
 *
 * Implementation walks the orchestration steps in the order specified
 * by the Phase 3 plan:
 *
 ...
 * "ephemeral by default" decision the Phase 3 roadmap commits to;
 * --keep is the only way to opt out.
 */
...
/* Diagnostic scratch shared with the rest of oci_run. Thread-local so
 * future parallel runs do not stomp each other. The buffer is sized
 * for the longest dynamic message (a quoted argv[0] plus the searched
 * PATH list propagated up from path-resolve).
 */
static _Thread_local char run_err_buf[2048];

static void set_err_static(const char **err, const char *msg)
{
    if (err)
        *err = msg;
}

static void set_err_fmt(const char **err, const char *fmt, ...)
...
static void set_err_fmt(const char **err, const char *fmt, ...)
{
    va_list ap;
    va_start(ap, fmt);
    vsnprintf(run_err_buf, sizeof(run_err_buf), fmt, ap);
    va_end(ap);
    if (err)
        *err = run_err_buf;
}
                     ...
                     const char **err)
{
    ...
    if (n < 0 || (size_t) n >= sizeof(path)) {
        set_err_static(err, "blob path too long");
        errno = ENAMETOOLONG;
        return -1;
    }
    int fd = open(path, O_RDONLY);
    if (fd < 0) {
        set_err_fmt(err, "cannot open blob %s: %s", path, strerror(errno));
        return -1;
    }
    struct stat st;
    if (fstat(fd, &st) < 0) {
        int saved = errno;
        close(fd);
        set_err_static(err, "fstat on blob failed");
        errno = saved;
        return -1;
    }
    if (st.st_size < 0 || st.st_size > 64 * 1024 * 1024) {
        close(fd);
        set_err_static(err, "blob too large");
        errno = EFBIG;
        return -1;
    }
    ...
    if (!buf) {
        close(fd);
        set_err_static(err, "out of memory loading blob");
        errno = ENOMEM;
        return -1;
    }
    size_t off = 0;
    while (off < want) {
        ...
        if (r < 0) {
            ...
            free(buf);
            close(fd);
            set_err_static(err, "read on blob failed");
            errno = saved;
            return -1;
        }
        if (r == 0)
    ...
    if (off != want) {
        free(buf);
        set_err_static(err, "short read on blob");
        errno = EIO;
        return -1;
    }
    buf[want] = '\0';
...
/* Recursive mkdir; tolerates existing intermediate directories. The
 * Phase 3 plan calls for chowning each newly created segment to
 * (spec.uid, spec.gid) when has_creds is set; macOS rejects fchownat
 * for non-root callers spoofing arbitrary uids, so the chown is best
 * effort and silently ignored when it fails. The intended ownership
 * is also recorded by the sidecar metadata that the syscall layer
 * (Phase 4) will read; for Phase 3 the host inode owner stays the
 * invoking user.
 */
...
{
    if (!path || !*path) {
        set_err_static(err, "empty path in mkdir_p");
        errno = EINVAL;
        return -1;
    }
    ...
    if (!dup) {
        set_err_static(err, "out of memory in mkdir_p");
        errno = ENOMEM;
        return -1;
    }
    for (char *p = dup + 1; *p; p++) {
        ...
        *p = '\0';
        if (mkdir(dup, mode) < 0 && errno != EEXIST) {
            int saved = errno;
            set_err_fmt(err, "mkdir %s failed: %s", dup, strerror(saved));
            free(dup);
            errno = saved;
            return -1;
        }
            ...
            const char **err)
{
    if (err)
        *err = NULL;
    if (!store || !ref || !opts) {
        set_err_static(err, "oci_run: NULL store/ref/opts");
        errno = EINVAL;
        return -1;
    }
    /* Suppress unused warnings for fields kept on the options struct
     * for forward compatibility with Phase 3 plan items not yet wired
    ...
    if (oci_unpack(store, ref, &uopts, &image_dir, &unpack_err) < 0) {
        set_err_fmt(err, "unpack failed: %s",
    ...
    if (oci_volume_ensure(opts->volume_dir, &volume_root, &vol_err) < 0) {
        set_err_fmt(err, "volume_ensure failed: %s",
    ...
    if (oci_store_get_ref(store, ref, &manifest_digest_str, &getref_err) < 0) {
        set_err_fmt(err, "no pin for ref: %s",
    ...
    if (oci_image_config_parse(config_body, config_len, &cfg, &cparse_err) <
        0) {
        set_err_fmt(err, "image config parse failed: %s",
    ...
    if (!cwd_host) {
        set_err_static(err, "out of memory building cwd host path");
        errno = ENOMEM;
        goto out;
    }
    ...
    if (!spec.argv || !spec.argv[0]) {
        set_err_static(err, "runspec produced empty argv");
        errno = EINVAL;
        goto out;
    }
    ...
    if (oci_path_resolve(run_dir, spec.argv[0], path_env, spec.cwd, &host_argv0,
        ...
        set_err_fmt(err, "%s", pr_err ? pr_err : "argv[0] resolution failed");
        goto out;
    }
     ...
     */
    if (chdir(cwd_host) < 0) {
        set_err_fmt(err, "chdir into '%s' failed: %s", cwd_host,
                    strerror(errno));
        goto out;
    }
...
{
    fputs(
        "usage: elfuse oci run [OPTIONS] IMAGE [ARG...]\n"
        "\n"
