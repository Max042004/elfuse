 ...
 *
 * Phase 1 keeps the model offline: parsers operate on in-memory JSON bytes
 * the caller already obtained from a registry fetch or disk fixture. The
 * registry client lives in a later slice; the manifest model exists now so
 * the fetch path can deserialize responses, and so the blob store can
 * persist the parsed graph without round-tripping through opaque JSON.
 *
...
/* Image config runtime block (the inner "config" object). Phase 3 of the
 * OCI roadmap consumes these fields; the model exists in Phase 1 to support
 * elfuse oci inspect rendering. NULL-terminated string arrays are NULL when
