...
static void test_media_type_predicates(void)
{
    ...
    CHECK(oci_media_type_is_config(OCI_MT_CONFIG_OCI), "predicate config OCI",
          NULL);
    CHECK(oci_media_type_is_layer(OCI_MT_LAYER_OCI_TAR_GZIP),
          "predicate layer", NULL);
...
static void test_manifest_oci_happy(void)
{
    ...
    CHECK(m.config.algo == OCI_DIGEST_SHA256, "manifest OCI config algo",
          NULL);
...
static void test_manifest_docker_happy(void)
{
    ...
    CHECK(m.media_type == OCI_MT_MANIFEST_DOCKER,
          "manifest Docker mediaType", NULL);
...
static void test_manifest_wrong_schema(void)
{
    ...
    CHECK(rc == -1 && err != NULL, "manifest schemaVersion != 2 rejected",
          err);
}
...
static void test_manifest_bad_digest(void)
{
    ...
    int rc = oci_manifest_parse(j, sizeof(j) - 1, &m, &err);
    CHECK(rc == -1 && err != NULL,
          "manifest uppercase / short digest rejected", err);
}
...
static void test_manifest_wrong_config_mediatype(void)
{
    ...
    int rc = oci_manifest_parse(j, sizeof(j) - 1, &m, &err);
    CHECK(rc == -1 && err != NULL,
          "manifest config descriptor with non-config mediaType rejected",
          err);
}
...
static void test_index_oci_pick_empty_variant(void)
{
    ...
    const char *err = NULL;
    int rc =
        oci_index_parse(OCI_INDEX_NO_V8, sizeof(OCI_INDEX_NO_V8) - 1, &idx,
                        &err);
    if (rc != 0) {
...
static void test_index_no_match_returns_null(void)
{
    ...
    int rc = oci_index_parse(OCI_INDEX_NO_LINUX_ARM64,
                             sizeof(OCI_INDEX_NO_LINUX_ARM64) - 1, &idx,
                             &err);
    if (rc != 0) {
...
static void test_index_skips_unknown_mediatype(void)
{
    oci_index_t idx;
    const char *err = NULL;
    int rc = oci_index_parse(OCI_INDEX_BAD_ARM64_MEDIATYPE,
                             sizeof(OCI_INDEX_BAD_ARM64_MEDIATYPE) - 1, &idx,
                             &err);
    if (rc != 0) {
...
static void test_image_config_happy(void)
{
    ...
    const char *err = NULL;
    int rc = oci_image_config_parse(OCI_IMAGE_CONFIG_GOOD,
                                    sizeof(OCI_IMAGE_CONFIG_GOOD) - 1, &c,
                                    &err);
    if (rc != 0) {
    ...
    CHECK(c.rootfs_diff_ids && c.rootfs_diff_ids[0] &&
              c.rootfs_diff_ids[1] && !c.rootfs_diff_ids[2],
...
static void test_image_config_missing_rootfs(void)
{
    ...
    CHECK(rc == -1 && err != NULL, "image config missing rootfs rejected",
          err);
}
