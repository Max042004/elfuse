 ...
 * nothing), plus the C3.1 direct-helper cases against a hand-rolled
 * uncompressed-tar blob in a tmp store, plus the C3.3c-ii raw-tar
 * populate and two-pass overlay-assembly path (single-file raw,
...
/* --- Plan 3 C3.3c-ii: raw populate + two-pass assembly ------------------ */
                            ...
                            size_t digest_str_cap)
{
    ...
    for (size_t i = 0; i < n_entries; i++) {
        ...
        append_entry(&body, e->path, e->size, e->mode, e->typeflag,
                     e->linkname, e->content);
    }
    bb_zero(&body, BLOCK * 2);
...
static void test_cross_image_stack_prefix_dedup(void)
{
    ...
    if (oci_store_stack_resolve(store, chain1, chain1_dir,
                                sizeof(chain1_dir)) < 0) {
    ...
    if (populate_raw_cache(name, store, bs, &d2b, ds2b, raw2b,
                           sizeof(raw2b)) < 0)
        goto close_store;
...
static void test_same_image_full_stack_short_circuits(void)
{
    ...
    if (!file_contents_match(p, "AA\n")) {
        report_fail(name, "stageB/a should be original (raw corruption "
                          "bypassed)");
        goto close_store;
    }
...
static void test_meta_sidecar_split_round_trip(void)
{
    ...
    const char *err = NULL;
    if (oci_meta_read_named(raw0, ".elfuse-meta.layer.json", &layer, &err) < 0) {
    ...
    if (oci_meta_read_named(raw1, ".elfuse-meta.layer.json", &layer, &err) < 0) {
    ...
    if (oci_meta_read_named(stage_dir, ".elfuse-meta.json", &roundtrip,
                            &err) < 0) {
