...
#   Binaries are cross-compiled from a pinned, unmodified source
#   snapshot by tests/fetch-fixtures.sh (Alpine ships no lmbench
#   package) into externals/test-fixtures/aarch64-musl/lmbench/;
#   static, so the same artifacts run in every environment. Each
#   metric is lmbench's own self-timed result (microseconds; lmbench
#   internally repeats -N times and reports the representative value).
...
# tests/lib/test-runner.sh (default here: 120s). Tier-1 self-times
# inside lmbench (BENCH_LMBENCH_TIMEOUT bounds each invocation); each
# Tier-2 metric runs BENCH_WARMUP + BENCH_ITERATIONS times, warmup
# samples are discarded, and the median of the rest is reported
...
#
# Tier-2 metrics are workload-only in every environment: the qemu,
# orbstack, and native columns time each sample inside the
# guest/machine via bench-timeit, and the elfuse column subtracts the
# median of the dedicated startup_ms metric (below) from every sample.
...
#   BENCH_LMBENCH_REPS     lmbench -N repetitions (default 11, lmbench's
#                          own TRIES default)
...
# section mirrors the issue #195 result format, with {"median": ...}
# leaves so results and baseline columns flatten the same way):
#   {
#     "schema": 1,
#     "meta": { "env", "host", "date", "iterations", "warmup", ... },
#     "lmbench": {
#       "lat_syscall": {
#         "null": {"median": <us>, "unit": "us"},
...
BENCH_LMBENCH_REPS="${BENCH_LMBENCH_REPS:-11}"
...
cleanup()
{
    ...
    if [ -n "$ORB_WORK" ]; then
        orb run rm -rf "$ORB_WORK" > /dev/null 2>&1 || true
    fi
    if [ -n "$MNT" ]; then
...
qemu_env_start()
{
    ...
    while :; do
        ...
        probe=$(qemu_exec env "ENOUGH=${BENCH_LMBENCH_ENOUGH:-100000}" \
...
bench_sample_us()
{
    ...
    start=$(epoch_us)
    timeout "$TEST_TIMEOUT" "$@" > /dev/null 2>&1 || rc=$?
    end=$(epoch_us)
    if [ "$rc" -ne 0 ]; then
        echo "bench-suite: sample '$label' failed (rc=$rc):" >&2
        echo "  $*" >&2
        exit 1
    fi
    echo $((end - start))
...
# keep a whole Tier-1 pass to a couple of minutes while still running
# every metric hundreds of thousands of iterations per interval. Set
...
lmbench_reap_stragglers()
{
    ...
    case "$BENCH_ENV" in
        ...
        native)
            ...
            ;;
        orbstack)
            orb run pkill -9 -f 'aarch64-musl/lmbench[/]' \
                > /dev/null 2>&1
            ;;
    esac
    return 0
...
lmbench_exec()
{
    if [ "$BENCH_ENV" = "qemu-aarch64" ]; then
        ...
        qemu_exec sh -c "${script} > /tmp/.lmbench.out 2>&1; rc=\$?; [ \$rc -ne 0 ] && pkill -9 -f 'aarch64-musl/lmbench[/]' 2> /dev/null; cat /tmp/.lmbench.out; rm -f /tmp/.lmbench.out; exit \$rc"
        return $?
    fi
...
# Runs "<binary> -N $BENCH_LMBENCH_REPS args..." and records the
# microseconds value from the "<label>: <value> microseconds" result
# line as "<benchmark> <metric> <value>". lmbench's benchmp harness
# forks coordination children every run, so a failed or hung attempt
# (rc != 0 or no result line) is retried BENCH_LMBENCH_RETRIES times
# and then recorded as an ERR row -- one wedged benchmark must not
# abort the suite or hide the other metrics.
lmbench_metric()
{
    ...
    for ((attempt = 0; attempt <= BENCH_LMBENCH_RETRIES; attempt++)); do
        ...
        bench_note "retrying $benchmark.$metric (rc=$rc)"
    done
    printf '%s %s ERR\n' "$benchmark" "$metric" >> "$LMROWS"
    {
        echo "bench-suite: WARNING: $benchmark.$metric failed in this environment (rc=$rc); recorded as ERR"
        printf '%s\n' "$out" | tail -3 | sed 's/^/    /'
    } >&2
    return 0
}
...
# with -1 columns for a failed phase. The 0k row is the pure-metadata
# create/delete cost the issue #195 schema tracks; convert ops/sec to
# microseconds per op to match the other Tier-1 metrics.
lmbench_lat_fs()
{
    local dir="$1"
    # lat_fs runs 8 benchmp rounds (4 file sizes x create+delete), each
    # ~1s per -N repetition by lmbench's iteration scaling, so a full
    # -N 11 pass legitimately needs ~100s+ even on native Linux. Scale
    # this benchmark's bound instead of inflating the default for the
    # single-round benchmarks.
    local BENCH_LMBENCH_TIMEOUT=$((BENCH_LMBENCH_TIMEOUT * 4))
    local attempt out row rc=0
    for ((attempt = 0; attempt <= BENCH_LMBENCH_RETRIES; attempt++)); do
        rc=0
        out="$(lmbench_exec "${LMBENCH_DIR}/lat_fs" \
            -N "$BENCH_LMBENCH_REPS" "$dir")" || rc=$?
        row="$(printf '%s\n' "$out" | awk '$1 == "0k" && NF == 4 \
            && $3 > 0 && $4 > 0 \
        ...
        bench_note "retrying lat_fs (rc=$rc)"
    done
    printf 'lat_fs create ERR\nlat_fs delete ERR\n' >> "$LMROWS"
    echo "bench-suite: WARNING: lat_fs failed in this environment (rc=$rc); recorded as ERR" >&2
    return 0
}
...
run_tier1_suite()
{
    ...
    case "$BENCH_ENV" in
        ...
        qemu-aarch64)
            ...
            fsdir="/tmp/bench-fs"
            {
                qemu_exec sh -c \
                    ...
                    && qemu_exec cp "${LMBENCH_DIR}/hello-s" /tmp/hello-s
        ...
        orbstack)
            ...
            probe="${fs_host}/probe"
            fsdir="${fs_host}/fs"
            orb run sh -c "mkdir -p ${fs_host}/fs \
    ...
    bench_note "lmbench: -N ${BENCH_LMBENCH_REPS} repetitions/benchmark"
    ...
    bench_note "lmbench: $ok_rows metrics OK"
    ...
    case "$BENCH_ENV" in
        ...
        qemu-aarch64)
            ...
            ;;
        orbstack)
            orb run sh -c "rm -rf ${fs_host} /tmp/hello-s" \
                > /dev/null 2>&1 || true
            ;;
    esac
    return 0
...
# Populate a corpus work area at host path $1: corpus copy plus the
# zstd input (the corpus concatenated to ~11 MiB).
prepare_corpus()
{
    local hostwork="$1"
    cp -R "$CORPUS" "${hostwork}/corpus"
    cat "${hostwork}/corpus"/*/*.src > "${hostwork}/zin"
    local i
    for i in 1 2 3 4 5 6; do
        cat "${hostwork}/zin" "${hostwork}/zin" > "${hostwork}/zin.2"
    ...
    case "$BENCH_ENV" in
        elfuse-aarch64)
            printf '%s\n' env "PATH=/bin:/usr/bin" "HOME=${GUEST_WORK}" \
                ...
                "$ELFUSE" --sysroot "$SYS" "/usr/bin/${tool}" "$@"
            ;;
        qemu-aarch64)
            printf '%s\n' env "PATH=/bin:/usr/bin" "HOME=${GUEST_WORK}" \
...
# Timed sample for the Linux-side environments: the workload runs under
# bench-timeit inside the guest/machine, so the printed integer covers
# spawn + run + exit of the workload only -- the environment's
# per-command entry cost (ssh session, orb run session, host spawn)
# stays outside the window and is reported separately as startup_ms.
# The repo-absolute bench-timeit path is resolved by the transport
# (rewritten to /mnt/host by qemu_exec; reached over the macOS share by
...
run_tier2_tool()
{
    ...
    for ((i = 0; i < total; i++)); do
        ...
        if [ "$BENCH_ENV" = "elfuse-aarch64" ]; then
            us=$(bench_sample_us "$metric" "${cmd[@]}") || exit 1
        else
...
# side -- the per-command entry toll of each environment. For elfuse
# (VM + sysroot setup + static ELF load, via busybox true) the
# application metrics subtract this metric's median per sample, so every
# column's Tier-2 numbers are workload-only and directly comparable; the
# other environments already exclude their entry cost via bench-timeit.
run_startup_metric()
{
    local total=$((BENCH_WARMUP + BENCH_ITERATIONS))
    ...
    for ((i = 0; i < total; i++)); do
        ...
        case "$BENCH_ENV" in
            elfuse-aarch64)
                ...
                ;;
            qemu-aarch64)
                qemu_exec true > /dev/null 2>&1 || rc=$?
                ;;
            native)
...
tier2_sh()
{
    case "$BENCH_ENV" in
        elfuse-aarch64)
            "$ELFUSE" --sysroot "$SYS" /bin/busybox sh -c "$1" \
                > /dev/null 2>&1
            ;;
        qemu-aarch64)
            qemu_exec sh -c "$1" > /dev/null 2>&1
            ;;
        native)
            sh -c "$1" > /dev/null 2>&1
            ;;
        orbstack)
            orb run sh -c "$1" > /dev/null 2>&1
            ;;
    esac
}
...
run_tier2_suite()
{
    ...
    case "$BENCH_ENV" in
        ...
        qemu-aarch64)
            ...
            if ! qemu_exec sh -c 'command -v python3 git rg zstd make' \
                > /dev/null 2>&1; then
            ...
            prep="$prep && cat /bench-work/corpus/*/*.src > /bench-work/zin"
            prep="$prep && for i in 1 2 3 4 5 6; do"
            prep="$prep cat /bench-work/zin /bench-work/zin > /bench-work/zin.2"
            prep="$prep && mv /bench-work/zin.2 /bench-work/zin; done"
            qemu_exec sh -c "$prep" > /dev/null 2>&1 \
        ...
        orbstack)
            ...
            prep="$prep && cat $ORB_WORK/corpus/*/*.src > $ORB_WORK/zin"
            prep="$prep && for i in 1 2 3 4 5 6; do"
            prep="$prep cat $ORB_WORK/zin $ORB_WORK/zin > $ORB_WORK/zin.2"
            prep="$prep && mv $ORB_WORK/zin.2 $ORB_WORK/zin; done"
...
emit_json()
{
    ...
    if [ "$BENCH_ENV" = "qemu-aarch64" ]; then
        ...
        note="application metrics are workload-only: the startup_ms median (VM + sysroot setup + static ELF load) is subtracted from every sample"
        adjust=1
    fi

    # lmbench provenance for meta: the pinned commit recorded by
    # fetch-fixtures.sh. Results published from these binaries must come
    # from unmodified sources (lmbench COPYING-2); the VERSION stamp is
    # the audit trail.
    local lmbench_src=""
...
# Tier-1 rows are lmbench's own self-timed results, one row per
# metric: "<benchmark> <metric> <value_us|ERR>". ERR rows keep only
# their status so a benchmark that cannot run in this environment is
# visible without polluting the numeric columns.
with open(os.environ["BENCH_LMBENCH_FILE"], encoding="utf-8") as fh:
    for line in fh:
        benchmark, metric, value = line.split()
        node = result["lmbench"].setdefault(benchmark, {})
        if value == "ERR":
            node[metric] = {"status": "ERR"}
        else:
            node[metric] = {"median": float(value), "unit": "us"}

# Tier-2 rows are wall-clock microseconds; report milliseconds.
# When BENCH_META_ADJUST is set (elfuse), the startup_ms median is
# subtracted from every application sample so the reported numbers are
# workload-only, matching the bench-timeit-based environments.
apps = data.get("applications", {})
startup_med_us = None
if os.environ.get("BENCH_META_ADJUST") and "startup_ms" in apps:
    vals = apps["startup_ms"]["startup_ms"]
    timed = vals[warmup:] if len(vals) > warmup else vals
    startup_med_us = statistics.median(timed)
for bench, rows in sorted(apps.items()):
    vals = rows[bench]
    if startup_med_us is not None and bench != "startup_ms":
        vals = [max(v - startup_med_us, 0.0) for v in vals]
