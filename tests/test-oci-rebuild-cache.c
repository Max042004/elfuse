/* OCI rebuild-cache unit tests (Plan 3 C3.5)
 *
...
static void stack_dir_for(const char *store_root, const char *chain_id,
                          char *out, size_t cap)
{
...
static void case_single_tree_commit(const char *scratch)
{
    ...
    snprintf(origin_in_stack, sizeof(origin_in_stack),
             "%s/.elfuse-origin.json", stack_dir);
